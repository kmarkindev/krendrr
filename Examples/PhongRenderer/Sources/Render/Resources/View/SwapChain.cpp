...
namespace kRendrr
{
    SwapChain::SwapChain(std::shared_ptr<kRendrr::RenderDevice> RenderDevice, HWND Hwnd)
        : RenderDevice(std::move(RenderDevice)), Hwnd(Hwnd), BuffersCount(2)
    {

    }

    void SwapChain::Initialize(const CommandQueue& CommandQueue)
    {
    ...
    bool SwapChain::HasRenderTarget() const
    {
        return !RenderTargets.empty();
    }
    ...
    void SwapChain::OnHwndChangedSize()
    {
        CheckInitialization();
        ...
        DxgiSwapChain
            ->ResizeBuffers(BuffersCount, 0, 0, DXGI_FORMAT_UNKNOWN, 0)
            >> HResultCheck {};

        ObtainRenderTargets();
    }

    void SwapChain::OnHwndShown()
    {
        ObtainRenderTargets();
    }

    void SwapChain::OnHwndHidden()
    {
        ReleaseRenderTargets();
    }
    ...
    void SwapChain::ReleaseRenderTargets()
    {
        ...
        RenderTargets.clear();
        CpuRenderTargetsHeap->Release();
    }

    void SwapChain::ObtainRenderTargets()
    {
        if(HasRenderTarget())
        {
            return;
