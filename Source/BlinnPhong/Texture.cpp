...
void krendrr::render::Texture::Load(const std::string_view& TextureFileName, const TextureLoadParams& Params)
{
    ...
    if(!Data)
        throw std::runtime_error("Failed to load texture. File is invalid.");
    ...
    switch (Channels)
    {
        ...
        break;
        default:
            throw std::runtime_error("Can't load texture, it has unsupported number of channels: " + std::to_string(Channels));
    }
